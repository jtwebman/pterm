...
export function BranchGroup({ project, branchId, name, folder, terminals }: Props) {
	...
	useEffect(() => {
		...
		return () => {
			cleanup();
			void bridge.git.unwatchBranch(folder);
		};
	}, [folder, dispatch]);
	...
	return (
		<div className="ml-2">
			...
			{!collapsed && terminals.length > 0 && (
				<div className="ml-2">
					{terminals.map((t) => (
						<TerminalTab key={t.key} terminal={t} isActive={t.key === state.activeTerminalKey} />
					))}
					<div
						onDragOver={(e) => {
							e.preventDefault();
