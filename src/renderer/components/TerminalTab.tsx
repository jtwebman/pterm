...
export function TerminalTab({ terminal, isActive }: Props) {
  ...
  const dotColor = terminal.status === "exited"
    ? "bg-gray-500"
    : (DOT_COLORS[terminal.activity] ?? "bg-gray-500");
  ...
  return (
    ...
    >
      ...
      <div className="truncate flex-1 min-w-0">
        ...
        {statusText && (
          <div className="truncate text-[0.7em] opacity-60">{statusText}</div>
        )}
      </div>
      <button
        onClick={handleClose}
