...
export function ProjectItem({ project, onEdit }: Props) {
	...
	const pendingBranches = state.pendingBranches.filter((b) => b.projectId === project.id);
	...
	return (
		<div className="mb-1">
			<div className="flex items-center gap-1 px-2 py-1.5 group">
				...
				<span className="truncate flex-1 text-gray-800 dark:text-gray-200 font-medium">
					{project.name}
				</span>
				<button
			...
			{!collapsed && (
				<>
					...
					{pendingBranches.map((pb) => (
						<div key={`pending-${pb.name}`} className="ml-2">
							<div className="flex items-center gap-1 px-2 py-1">
								<span className="w-3 shrink-0" />
								<span className="truncate flex-1 text-gray-400 dark:text-gray-500 text-xs font-medium animate-pulse">
									{pb.name}
								</span>
			...
			{showBranchManager && (
				<BranchManager project={project} onClose={() => setShowBranchManager(false)} />
			)}
		</div>
	);
}
