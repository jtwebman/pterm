...
function detectShellActivity(ptyPid: number): Promise<ActivityUpdate> {
  return new Promise((resolve) => {
    if (process.platform === "win32") {
      execFile(
        "powershell",
        ["-NoProfile", "-Command", `Get-CimInstance Win32_Process -Filter "ParentProcessId=${ptyPid}" | Select-Object -First 1`],
        (err, stdout) => {
...
async function detectClaudeActivity(ptyPid: number, activityFile?: string): Promise<ActivityUpdate> {
  try {
    if (activityFile) {
...
function readActivityFile(filePath: string): ActivityUpdate | null {
  try {
...
function processHasChildren(pid: number): Promise<boolean> {
