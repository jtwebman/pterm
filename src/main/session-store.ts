...
import type { CommandType, SavedSession } from "../shared/types.js";
...
export class SessionStore {
	...
	loadAllSessions(): SavedSession[] {
		const rows = this.stmtLoadAllSessions.all() as Record<string, unknown>[];
		return rows.map((r) => ({
			terminalId: r.terminal_id as string,
			projectId: r.project_id as string,
			commandId: (r.command_id as string | null) ?? undefined,
			branchId: (r.branch_id as string | null) ?? undefined,
			commandName: r.command_name as string,
			commandType: r.command_type as CommandType,
			status: r.status as "running" | "exited",
			exitCode: (r.exit_code as number | null) ?? undefined,
			cwd: r.cwd as string,
			cols: r.cols as number,
			rows: r.rows as number,
			aiSessionId: (r.ai_session_id as string | null) ?? undefined,
		}));
	}
	...
	loadScrollback(terminalId: string): Buffer[] {
		const rows = this.stmtLoadScrollback.all(terminalId) as unknown as { data: ArrayBuffer }[];
