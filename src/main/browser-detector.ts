...
function getWindowsUsers(): string[] {
	try {
		...
		return entries.filter((e) => !["Public", "Default", "Default User", "All Users"].includes(e));
	} catch {
		return [];
	}
}
...
function linuxCandidates(): BrowserCandidate[] {
	...
	const candidates: BrowserCandidate[] = [
		...
		},
		{
			...
			localStatePaths: [join(home, ".config/BraveSoftware/Brave-Browser/Local State")],
		},
	];
	...
	if (isWsl()) {
		...
		candidates.push({
			name: "Windows Chrome",
			type: "chrome",
			commands: ["/mnt/c/Program Files/Google/Chrome/Application/chrome.exe"],
			localStatePaths: users.map(
				(u) => `/mnt/c/Users/${u}/AppData/Local/Google/Chrome/User Data/Local State`,
			),
		});
		candidates.push({
			name: "Windows Edge",
			type: "edge",
			commands: ["/mnt/c/Program Files (x86)/Microsoft/Edge/Application/msedge.exe"],
			localStatePaths: users.map(
				(u) => `/mnt/c/Users/${u}/AppData/Local/Microsoft/Edge/User Data/Local State`,
			),
		});
		candidates.push({
	...
	return [
		{
			name: "Google Chrome",
			type: "chrome",
			commands: ["/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"],
			localStatePaths: [join(home, "Library/Application Support/Google/Chrome/Local State")],
		},
		{
			name: "Firefox",
			type: "firefox",
		...
		},
		{
			name: "Microsoft Edge",
			type: "edge",
			commands: ["/Applications/Microsoft Edge.app/Contents/MacOS/Microsoft Edge"],
			localStatePaths: [join(home, "Library/Application Support/Microsoft Edge/Local State")],
		},
		{
			name: "Brave Browser",
			type: "brave",
			commands: ["/Applications/Brave Browser.app/Contents/MacOS/Brave Browser"],
			localStatePaths: [
...
async function readProfiles(localStatePaths: string[]): Promise<BrowserProfile[]> {
	for (const p of localStatePaths) {
		try {
			...
			for (const [dir, info] of Object.entries(cache)) {
				const profile = info as Record<string, unknown>;
				const name =
					(typeof profile?.name === "string" ? profile.name : null) ||
					(typeof profile?.shortcut_name === "string" ? profile.shortcut_name : null) ||
					dir;
...
export async function detectBrowsers(): Promise<DetectedBrowser[]> {
	...
	const candidates = platform === "darwin" ? macCandidates() : linuxCandidates();
