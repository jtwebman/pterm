...
contextBridge.exposeInMainWorld("ptermBridge", {
	terminal: {
		...
		onData: (terminalId: string, cb: (data: string) => void) => {
			ipcRenderer.on(`terminal:data:${terminalId}`, (_event: IpcRendererEvent, data) => cb(data));
		},
		...
		onExit: (terminalId: string, cb: (data: { exitCode: number; signal?: number }) => void) => {
			ipcRenderer.on(`terminal:exit:${terminalId}`, (_event: IpcRendererEvent, data) => cb(data));
		},
		...
		onActivity: (terminalId: string, cb: (data: ActivityUpdate) => void) => {
			ipcRenderer.on(`terminal:activity:${terminalId}`, (_event: IpcRendererEvent, data) =>
				cb(data),
			);
		},
	...
	git: {
		...
		onBranchChanged: (cb: (folder: string, branch: string) => void) => {
			const handler = (_event: IpcRendererEvent, folder: string, branch: string) =>
				cb(folder, branch);
			ipcRenderer.on("git:branch-changed", handler);
			return () => {
				ipcRenderer.removeListener("git:branch-changed", handler);
			};
		},
	},
	theme: {
